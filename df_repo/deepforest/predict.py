...
def predict_file(model, csv_file, root_dir, savedir, device, iou_threshold=0.1, color=(0,165,255), thickness=1):
    ...
    with torch.no_grad():
        for i in ds:
            i = i.to(device)
            prediction = model.forward(torch.unsqueeze(i,0))
            
            # $$ instead maybe make retinanet only output that -- when increasing complexity
            # make it take only the highest label and score (manually supply bbox)
            _,h,w, = i.shape
            temp_dict = {}
            temp_dict['boxes'] = torch.tensor([[0,0,h,w]])

            # find max score 
            # print(prediction[0]['boxes'])
            # print(temp_dict['boxes'])
            # exit(0)
            max_idx = torch.argmax(prediction[0]['scores'])
            temp_dict['scores'] = torch.tensor([prediction[0]['scores'][max_idx]])
            temp_dict['labels'] = torch.tensor([prediction[0]['labels'][max_idx]])

            prediction[0] = temp_dict
...
def predict_tile(model,
    ...
    if len(predicted_boxes) == 0:
        print("No predictions made, returning None: C")
        return None
    if mosaic:
