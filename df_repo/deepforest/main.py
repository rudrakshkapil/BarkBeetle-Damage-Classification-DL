...
class deepforest(pl.LightningModule):
    ...
    def predict_tile(self,
        ...
        if result is None:
            print("No predictions made, returning None: B")
            return None
    ...
    def training_step(self, batch, batch_idx):
        ...
        for key, value in loss_dict.items():
            self.log("train_{}".format(key), value, on_epoch=True, batch_size=self.config["batch_size"])     
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Log loss
        for key, value in loss_dict.items():
            self.log("val_{}".format(key), value, on_epoch=True, batch_size=self.config["batch_size"])     
    ...
    def on_epoch_end(self):
        if not self.config["validation"]["csv_file"] == None:
            if (self.current_epoch + 1) % self.config["validation"]["val_accuracy_interval"] == 0 :
                ...
                self.log("box_recall",results["box_recall"], batch_size=self.config["batch_size"]) 
                self.log("box_precision", results["box_precision"], batch_size=self.config["batch_size"])
                
                if not type(results["class_recall"]) == type(None):
                    for index, row in results["class_recall"].iterrows():
                        self.log("{}_Recall".format(self.numeric_to_label_dict[row["label"]]),row["recall"], batch_size=self.config["batch_size"])
                        self.log("{}_Precision".format(self.numeric_to_label_dict[row["label"]]),row["precision"], batch_size=self.config["batch_size"])
    ...
    def configure_optimizers(self):
        if self.config["optim"] == 'sgd':
            ...
            optimizer = optim.Adadelta(self.model.parameters(), lr=self.config["train"]["lr"])
